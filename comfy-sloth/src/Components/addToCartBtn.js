...
export function AddToCartBtn(props) {
    ...
    async function changeProductCount(count) {
        setLoading(true)
        let requestInfo = {
            username,
            token,
            id: params.productId ? params.productId : props.productId,
            count: count === undefined ? productCount !== null ? productCount : 0 : count
        }
        try {
        ...
        {maxCount === 0 ?
            <p style={{fontWeight: 500}} className={"text-danger text-center"}>this product does not exist any
                ...
                <button className={"buy-btn btn "} style={{width: "7rem"}}
                    ...
                    {loading ? (<svg className={'text-dark fs-1'} xmlns="http://www.w3.org/2000/svg" width="24"
                                     height="24"
                                     viewBox="0 0 24 24">
                    ...
                    <FaTrash className={"me-3 user-select-none text-muted"}
                             onClick={() => {
                                 ...
                                 //when user removes a product this function runs
                                 // changeBasket()
                             }}
